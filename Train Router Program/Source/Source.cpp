...
//If you want to actually finish the program, leave this false
bool detailedExec = false;
...
//FILE PARSING INFO CONTAINERS
	...
	{
		float hubFuel = 10000.00;		// :^) not used anymore but don't delete it
...
//TRAIN PROCESSING METHODS AND SIMULATION METHODS
	...
	void assignLoads(simulationParameters param, milTime time, int numHubs, int dispatchTime)
	{
		...
		{
			int soonest = loads.size() - 1; //Already sorted backwards by most recent
			...
			{
				//cout << endl << "No trains available, storing " << loads[soonest]->getName() << endl;
			...
			{
				//cout << endl << "Trying to dispatch load " << loads[soonest]->getName() << endl;
				...
				for (int i = 0; i < numHubs; i++) //For every hub on the map, starting with the nearest to the pickup point
				{
					...
					for (int t = 0; t < candidateHub->getNumTrainsHere(); t++) //For every train at the current hub
					{
						...
						//cout << "Evaluating " << current->getName() << "(" << current->getStateName() << ")" << endl;

						if (current->getType() != loads[soonest]->getType())
						{
							//cout << "	Skipping incorrect train type" << endl;
							continue; //Skip trains of incorrect type
						}
					...
					//else cout << "Failed to find train in hub for load " << loads[soonest]->getName() << endl;
				}
		...
		//Place storage back into global loads list
...
int main()
{
	...
	initRepFreights.push_back({  "STATION6",  "STATION1", "F", "04 55",  "50" });
	...
	initRepPassengers.push_back({ "W", initPRoutesOne });
	...
	initDayFreight.push_back({ "19" ,"STATION7",  "STATION4", "13 01",  "11" });
	...
	initPRoutesThree.push_back({ "STATION11", "12 50" }); 
	...
	initDayPass.push_back({ "18", initPRoutesThree });
	...
	cout << rule << "Pre-calculating hub-station maximum distances..." << endl;
	...
	//Print railway info
	cout << rule << "RAILWAY DETAILS" << endl << rule << endl;
	...
	while (simDay <= parameters.duration)
	{
		...
		if (detailedExec == true)
		{
			printDay();
			printTime(realTime);
			printAllTrains();
			//printAllLoads();
			bool skipIdleTime = true;
			for (int t = 0; t < trains.size(); t++)
			{
				train* current = trains[t];
				if (current->getState() != IDLE && current->getState() != MAIN)
				{
					skipIdleTime = false; break;
				}
			}
			//if (trainsBoring == false) system("pause");
			if (skipIdleTime == false) _sleep(80);
			system("cls");
			cout << endl;
		}
